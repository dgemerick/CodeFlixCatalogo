...
public class ListCategoriesTest
{
    ...
    public async Task List()
    {
        ...
        var outputRepositorySearch = new SearchOutput<DomainModel.Category>(
            currentPage: input.Page,
            perPage: input.PerPage,
            items: (IReadOnlyList<DomainModel.Category>)categoriesExampleList,
    ...
    public async Task ListOkWhenEmpty()
    {
        ...
        var outputRepositorySearch = new SearchOutput<DomainModel.Category>(
            currentPage: input.Page,
            perPage: input.PerPage,
            items: new List<DomainModel.Category>().AsReadOnly(),
            total: 0
        );
        repositoryMock.Setup(x => x.Search(
            It.Is<SearchInput>(
    ...
    [Theory(DisplayName = nameof(ListInputWithoutAllParamenters))]
    [Trait("Application", "ListCategories - use Cases")]
    [MemberData(
        nameof(ListCategoriesTestDataGenerator.GetInputWithoutAllParameter),
        parameters: 14,
        MemberType = typeof(ListCategoriesTestDataGenerator)
    )]
    public async Task ListInputWithoutAllParamenters(UseCase.ListCategoriesInput input)
    {
        var categoriesExampleList = _fixture.GetExampleCategoriesList();
        var repositoryMock = _fixture.GetRepositoryMock();
        var outputRepositorySearch = new SearchOutput<DomainModel.Category>(
